...
select get_json_object('{"common":{"ar":"440000","ba":"iPhone","ch":"Appstore","md":"iPhone Xs","mid":"mid_904","os":"iOS 13.3.1","uid":"325","vc":"v2.1.134"},"displays":[{"displayType":"query","item":"6","item_type":"sku_id","order":1},{"displayType":"promotion","item":"6","item_type":"sku_id","order":2},{"displayType":"query","item":"2","item_type":"sku_id","order":3},{"displayType":"promotion","item":"1","item_type":"sku_id","order":4},{"displayType":"query","item":"5","item_type":"sku_id","order":5}],"page":{"during_time":3202,"item":"荣耀手机","item_type":"keyword","last_page_id":"search","page_id":"good_list"},"ts":1592211744361}','$.displays[0].order');
...
--创建永久函数
create function explode_json_array as 'com.atguigu.gmall.hive.udtf.ExplodeJsonArray' using jar 'hdfs://hadoop102:8020/user/hive/jars/gmall-1109-1.0-SNAPSHOT.jar';
...
select
    ...
    sku.create_time
from
(select * from ods_sku_info where dt='2020-06-14')sku
join
(select * from ods_base_trademark where dt='2020-06-14')ob on sku.tm_id=ob.tm_id
join
(select * from ods_spu_info where dt='2020-06-14')spu on spu.id=sku.spu_id
join
(
...
tblproperties ("parquet.compression"="lzo");
drop table dwd_dim_date_info_tmp;
--数据装载
...
select
    ...
    od.source_type,
    od.source_id,
    od.sku_num * od.order_price as original_amount_d,--商品原始价格
    od.sku_num * od.order_price / oi.original_total_amount * oi.final_total_amount final_amount_d,--商品购买价格分摊
    od.sku_num * od.order_price / oi.original_total_amount * oi.feight_fee feight_fee_d,--运费分摊
    od.sku_num * od.order_price / oi.original_total_amount * oi.benefit_reduce_amount benefit_reduce_amount_d --优惠分摊
from
...
select
        ...
        if(rk=1,final_total_amount-final_amount_d_sum+final_amount_d,final_amount_d) final_amount_d,
        if(rk=1,benefit_reduce_amount-benefit_reduce_amount_d_sum+benefit_reduce_amount_d) benefit_reduce_amount_d
from
(
    select
        od.id,
        ...
        od.source_id,
        od.sku_num * od.order_price as original_amount_d,--商品原始价格
        round(od.sku_num * od.order_price / oi.original_total_amount * oi.feight_fee,2) feight_fee_d,--运费分摊
        round(od.sku_num * od.order_price / oi.original_total_amount * oi.final_total_amount,2) final_amount_d, --商品购买价格分摊
        round(od.sku_num * od.order_price / oi.original_total_amount * oi.benefit_reduce_amount,2)  benefit_reduce_amount_d, --优惠分摊
        ...
        sum(round(od.sku_num * od.order_price / oi.original_total_amount * oi.feight_fee,2)) over (partition by od.order_id) feight_fee_d_sum,
        sum(round(od.sku_num * od.order_price / oi.original_total_amount * oi.final_total_amount,2)) over (partition by od.order_id) final_amount_d_sum,
        sum(round(od.sku_num * od.order_price / oi.original_total_amount * oi.benefit_reduce_amount,2)) over (partition by od.order_id) benefit_reduce_amount_d_sum,

        rank() over (partition by od.order_id order by od.id) rk  --按照订单号order_id分区，明细号排序id
    from
    (select * from ods_order_detail where dt='2020-06-14') od
    join
    (select * from ods_order_info where  dt='2020-06-14') oi
...
select
    ...
    nvl(old.coupon_status,new.coupon_status),
    nvl(old.get_time,new.get_time),
    nvl(old.using_time,new.using_time),
    nvl(old.used_time,new.used_time),
    date_format(nvl(old.get_time,new.get_time),'yyyy-MM-dd')
from
(select
        id,
        coupon_id,
...
select
    ...
    where dt in
    (
        select date_format(ods_order_info.create_time, 'yyyy-MM-dd')
        from ods_order_info
        where dt = '2020-06-14'
    )
) ;
...
select
    '2020-06-15', --统计日期
    date_add('2020-0-15',-1), --留存天数
    sum(if(login_date_first=('2020-06-15',-1)and login_date_last='2020-06-15',1,0)), --14号一日留存用户数
    sum(if(login_date_first=date_add('2020-06-15',-1),1,0)), --14号新增用户数
    sum(if(login_date_first=('2020-06-15',-1)and login_date_last='2020-06-15',1,0)) / sum(if(login_date_first=date_add('2020-06-15',-1),1,0)) *100  --14号一日用户留存率
from dwt_uv_topic;
...
select
    '2020-06-25',
    concat(date_add('2020-06-25',-6),'-','2020-06-25'),
    count(*)
from
    (
    select
    ...
    (
        select
            mid_id,
            dt,
            lead(dt,2,'9999-99-99') over (partition by mid_id order by dt) lead_dt
        from
        (
            select
                mid_id,
...
select
    ...
    sum(if(login_date_last='2020-06-25',1,0)) / sum(if( login_count>0,1,0)) *100,
...
select
    ...
    tmp_cop.order_count,
    nvl(tmp_cop.order_count / tmp_page.good_detail_count * 100,0),
...
(
    ...
    group by tm_id,category1_name,category1_id,user_id
) t1
